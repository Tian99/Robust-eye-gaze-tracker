...
def hierarchy(image_1, image_2, reduced_1, reduced_2):
    ...
    while k >= 0:
        ...
        if k == n:
            ...
            U = 4*a_4_pyramid.expand_single(U)
            V = 4*a_4_pyramid.expand_single(V)
        #Stack U and V
        U_V = np.dstack((U,V))
...
image_1 = cv2.imread('input/TestSeq/Shift0.png')
image_2 = cv2.imread('input/TestSeq/ShiftR40.png')
...
reduced_1 = a_4_pyramid.reduce(image_1)
reduced_2 = a_4_pyramid.reduce(image_2)
...
new_y_pos = [x-40 for x in new_y_pos]
new_x_pos = [y-20 for y in new_x_pos]
