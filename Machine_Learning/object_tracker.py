...
class auto_tracker():
	def __init__(self, tracker = "kcf", cropped):
		super().__init__()
		self.pupil_x = []
		self.pupil_y = []
		self.pupil_count = []
		self.count = 0
		self.iniBB = cv2.imread('../input/chosen_pic.png')
		...
		while True:
			...
			(successm, box) = tracker.update(frame)
			...
			if success:
				...
				for(i, (k, v)) in enumerate(info):
					...
					cv2.putText(frame, text, (10, H - ((i*20) + 20)), 
							cv2.FRONT_HERSHEY_SIMPLEX, 0.6, (0,0,255), 2)
